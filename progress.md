...
## 2026-04-26 - Fix multi-tenant `timing_s/param_sync` (~15s → ~2s)

- `MultiTenantTrainer._fit_update_weights` previously did two back-to-back syncs inside the `param_sync` timer:
  1. `checkpoint_manager.update_weights(global_steps=...)` — fast NCCL transfer (~2s) but landed weights in the hardcoded single-tenant `VLLM_LORA_INT_ID = 1` slot, so the data was wasted (per-tenant generation uses different slots). Was originally kept just to set `vllm.global_steps` (see 2026-03-23 fix).
  2. `_sync_tenant_lora_to_rollout()` — the actual per-tenant sync, going through FSDP all-gather → CPU copy → Ray RPC → `cloudpickle.dumps` → `collective_rpc` → `cloudpickle.loads` per vLLM worker. ~13s on Qwen-7B rank-32, dominating the step.
- Root cause: NCCL `_update_weights` had `lora_int_id=VLLM_LORA_INT_ID` hardcoded ([verl/workers/rollout/vllm_rollout/utils.py](verl/workers/rollout/vllm_rollout/utils.py)), so the fast path could not be reused for per-tenant routing — forcing the slow Ray-RPC fallback.
- Fix: plumbed `lora_int_id` (and `peft_config`) through the existing NCCL `update_weights` chain so each tenant's adapter lands directly in its own vLLM slot in a single transfer, then deleted `_sync_tenant_lora_to_rollout`. Backward-compatible: `lora_int_id=None` (single-tenant) keeps original behavior.
- Changes:
  - [verl/workers/engine_workers.py](verl/workers/engine_workers.py): `ActorRolloutRefWorker.update_weights` accepts `lora_only=False`; when set, `get_per_tensor_param(base_sync_done=True)` ships LoRA-only tensors instead of the full base model.
  - [verl/checkpoint_engine/base.py](verl/checkpoint_engine/base.py): `CheckpointEngineManager.update_weights` and `CheckpointEngineWorker.update_weights` accept `lora_int_id` and `peft_config`; when both are set, `lora_only=True` is forwarded to the trainer side and `peft_config` + `base_sync_done=True` + `lora_int_id` are forwarded to the rollout side.
  - [verl/workers/rollout/vllm_rollout/utils.py](verl/workers/rollout/vllm_rollout/utils.py): `update_weights_from_ipc` and `_update_weights` accept `lora_int_id` and use it for the `TensorLoRARequest`/`remove_lora` calls (defaulting to `VLLM_LORA_INT_ID` when not provided).
  - [verl/experimental/fully_async_policy/multi_tenant_trainer.py](verl/experimental/fully_async_policy/multi_tenant_trainer.py): `_fit_update_weights` makes a single `checkpoint_manager.update_weights(global_steps, lora_int_id, peft_config)` call inside the timer; `peft_config` is cached in `init_tenant_adapters_on_rollout`. Removed `_sync_tenant_lora_to_rollout`. The startup `add_tenant_lora` Ray-RPC path is kept for the one-time initial adapter registration on vLLM (not perf-critical).
