...
_SYSTEM_METRIC_PREFIXES = (
    ...
    "decoupled/monitor/",
    "decoupled/static/",
    "decoupled/count/staleness_",
    "decoupled/count/total_generated_samples",
    "decoupled/count/dropped_stale_samples",
)
_SYSTEM_METRIC_EXACT = {"decoupled/total_wait_time"}
...
def assemble_batch_from_rollout_samples(
    ...
    rollout_status = {f"decoupled/{key}": value for key, value in rollout_status.items()}
    ...
    processing_time_stats = {f"decoupled/{key}": value for key, value in processing_time_stats.items()}
    ...
    partial_stats = {
        "decoupled/partial/total_partial_num": len(param_version_diff) - num_diff0,
        "decoupled/partial/partial_ratio": (len(param_version_diff) - num_diff0) / len(param_version_diff),
        "decoupled/partial/max_partial_span": max(param_version_diff),
    }
    # add meta_info
...
class MetricsAggregator:
    ...
    def _init_aggregation_rules(self) -> dict[str, dict[str, list[str]]]:
        ...
        return {
            ...
            "last": [
                "decoupled/count/total_generated_samples",
                "decoupled/count/stale_samples_processed",
                "decoupled/count/stale_trajectory_processed",
                "decoupled/count/current_param_version",
                "decoupled/count/dropped_stale_samples",
    ...
    def _special_metrics_aggergate(self, aggregated: dict[str, Any]) -> dict[str, Any]:
        ...
        if "timing_s/gen" in aggregated.keys() and "timing_s/step" in aggregated.keys():
            aggregated["decoupled/trainer/idle_ratio"] = aggregated["timing_s/gen"] / aggregated["timing_s/step"]
